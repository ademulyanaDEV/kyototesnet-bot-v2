...
def kirim_transaksi_dengan_delay(account, nonce):
    jumlah_kirim_acak = decimal.Decimal(0)  # Mengirim 0 ETH
    jumlah_kirim_acak_wei = int(jumlah_kirim_acak * 10**18)
    alamat_penerima = ""  # Alamat penerima tetap
    transaksi = {
        'nonce': nonce,
        'to': alamat_penerima,
...
def proses_kirim_transaksi_per_akun(account):
    ...
    for i in range(1, JUMLAH_TRANSAKSI_PER_AKUN + 1):
        try:
            ...
            saldo_diperlukan = decimal.Decimal(BATAS_GAS) * decimal.Decimal(MAX_FEE_PER_GAS) / 10**18
            ...
            hash_tx = kirim_transaksi_dengan_delay(account, nonce)
            if hash_tx:
                log(f"Transaction {i} sent. Account: {account['address']}, Nonce: {nonce}, Receiver Address: 0x00000000000000000000000000000000000A4B05, Hash: {hash_tx}", "SUCCESS")
                delay = 3  # Fixed delay before next transaction
